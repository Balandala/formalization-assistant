...
function getFormattingOptions() {
    ...
    return {
        ...
        tocPageNumber:
            Number.isNaN(tocPageNumber) || tocPageNumber < 1
                ? 2
                : tocPageNumber,
    };
}
...
function updateMode() {
    ...
    if (modeTitle.checked) {
        ...
        dropZoneText.textContent =
            "Кликните, чтобы сгенерировать титульный лист";
    } else {
        dropZoneText.textContent =
            "Перетащите сюда файл .docx или кликните, чтобы выбрать";
...
function renderCompletedState(docId, report) {
    ...
    const previewLabel = checkOnly
        ? "Предпросмотр исходного файла"
        : "Предпросмотр";
    const diffButton =
        report && report.diff_path
            ? `
...
async function uploadWithCover(
    file,
    formattingOptions = getFormattingOptions(),
) {
    const formData = new FormData();
    ...
    statusDiv.innerHTML =
        '<div class="alert alert-info">Загрузка и генерация титульного листа...</div>';
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(
                await readErrorMessage(
                    response,
                    "Ошибка при загрузке с титульным листом",
                ),
            );
        }
...
async function generateTitle() {
    ...
    statusDiv.innerHTML =
        '<div class="alert alert-info">Генерация титульного листа...</div>';
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(
                await readErrorMessage(response, "Ошибка генерации"),
            );
        }
...
function renderReport(report) {
    ...
    if (report.paragraphs_formatted > 0)
        items.push(
            `<li class="list-group-item"><i class="bi bi-check-circle text-success"></i> ${checkOnly ? "Будет нормализовано абзацев" : "Отформатировано параграфов"}: <strong>${report.paragraphs_formatted}</strong></li>`,
        );
    if (report.headings_detected > 0)
        items.push(
            `<li class="list-group-item"><i class="bi bi-check-circle text-success"></i> ${checkOnly ? "Будет оформлено заголовков" : "Определено заголовков"}: <strong>${report.headings_detected}</strong></li>`,
        );
    if (report.heading_page_breaks_added > 0)
        items.push(
            `<li class="list-group-item"><i class="bi bi-check-circle text-success"></i> ${checkOnly ? "Будет добавлено разрывов страницы перед заголовками" : "Добавлено разрывов страницы перед заголовками"}: <strong>${report.heading_page_breaks_added}</strong></li>`,
        );
    if (report.figures_numbered > 0)
        items.push(
            `<li class="list-group-item"><i class="bi bi-check-circle text-success"></i> ${checkOnly ? "Будет пронумеровано рисунков" : "Пронумеровано рисунков"}: <strong>${report.figures_numbered}</strong></li>`,
        );
    if (report.tables_numbered > 0)
        items.push(
            `<li class="list-group-item"><i class="bi bi-check-circle text-success"></i> ${checkOnly ? "Будет пронумеровано таблиц" : "Пронумеровано таблиц"}: <strong>${report.tables_numbered}</strong></li>`,
        );
    if (report.page_numbering_added)
        items.push(
            `<li class="list-group-item"><i class="bi bi-check-circle text-success"></i> ${checkOnly ? "Будет добавлена нумерация страниц" : "Добавлена нумерация страниц"}</li>`,
        );
    if (report.page_fields_set)
        items.push(
            `<li class="list-group-item"><i class="bi bi-check-circle text-success"></i> ${checkOnly ? "Будут установлены поля страницы" : "Установлены поля страницы"}</li>`,
        );
    if (report.table_of_contents_generated)
        items.push(
            `<li class="list-group-item"><i class="bi bi-check-circle text-success"></i> ${checkOnly ? "Будет сгенерировано содержание" : "Сгенерировано содержание"}${report.table_of_contents_entries > 0 ? `: <strong>${report.table_of_contents_entries}</strong>` : ""}${report.table_of_contents_page ? `, страница <strong>${report.table_of_contents_page}</strong>` : ""}</li>`,
        );

    if (items.length === 0 && (!report.details || report.details.length === 0))
        return "";
    ...
    if (report.details && report.details.length > 0) {
        const detailItems = report.details
            .map(
                (detail) =>
                    `<li class="list-group-item list-group-item-light small">${escapeHtml(detail)}</li>`,
            )
            .join("");
        detailsHtml = `
            <div class="mt-2">
